...
if st.sidebar.button('Prediksi'):
    ...
    forecast = model.make_future_dataframe(data, periods=7, n_historic_predictions=len(data))
    ...
    # Set a different color for the prediction graph
    prediction_color = 'lightblue'

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=hasil['tanggal_mendatang'], y=hasil['prediksi'], name="Hasil Prediksi", line_color=prediction_color))
    st.plotly_chart(fig)
